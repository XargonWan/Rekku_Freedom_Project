...
The new engine processes JSON messages generated by the LLM. Each message can contain multiple **actions**, for example:`message`, `save_memory`, or `bash`. The engine validates the structure and dispatches every action to a plugin located in the `plugins/` folder.

Example format:
```json
{
  "actions": [
    {"action": "message", "params": {"chat_id": 123, "text": "Hello"}},
    {"action": "save_memory", "params": {"content": "greeting", "tags": ["chat"]}}
  ]
}
```
